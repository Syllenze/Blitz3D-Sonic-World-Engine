Type Template
	...
	Field tex, animtex, loadedtex, texframe#, maxtexframes, texspeed#	;Texture
...
Function SetTemplateLoadedTexture(template, texno)
tmp.Template = Object.Template(template)
tmp\loadedtex = True
tmp\tex = texno
End Function
...
Function SetTemplateLoadedAnimTexture(template, texno, maxframes, speed# = 1)
tmp.Template = Object.Template(template)
tmp\loadedtex = True
tmp\animtex = True
tmp\maxtexframes = maxframes
...
Function SetTemplateSizeVel(template, size_add#=0, size_sub#=0, size_mult#=0)
tmp.Template = Object.Template(template)
...
Function SetTemplateRGB(template, r=255, g=255, b=255, r2=255, g2=255, b2=255)
tmp.Template = Object.Template(template)
tmp\realrgbenabled=True
tmp\r1 = r
tmp\g1 = g
tmp\b1 = b
tmp\r2 = r2
...
Select e\tmp\loadedtex
	Case false: If e\tmp\tex Then EntityTexture e\ent, e\tmp\tex
	Case true: If PARTICLETEXTURES(e\tmp\tex) Then EntityTexture e\ent, PARTICLETEXTURES(e\tmp\tex)
End Select
For i = 0 To 7
...
Function UpdateParticles(d.tDeltaTime)
For e.Emitter = Each Emitter
	If e\tmp\max_particles > -1 Then
		cnt_particles = 0
		For p.Particle = Each Particle
			If p\emitter = e Then cnt_particles = cnt_particles + 1*d\Delta
		Next
	EndIf
	ClearSurface e\surf
	If e\max_time > -1 Then
		If e\age > e\max_time Then e\del = True Else e\age = e\age + 1*d\Delta
	EndIf
		...
		If e\cnt_loop = 0 And e\del = False Then
			For i = 1 To e\tmp\particles_per_interval
				If (e\tmp\max_particles > -1 And cnt_particles < e\tmp\max_particles) Or e\tmp\max_particles = -1 Then
					...
					If e\tmp\faceowner Then
						p\xv# = Sin(EntityYaw#(e\owner, True))*Rnd(e\tmp\min_xv#, e\tmp\max_xv#)
						p\yv# = Rnd(e\tmp\min_yv#, e\tmp\max_yv#)
						p\zv# = -Cos(EntityYaw#(e\owner, True))*Rnd(e\tmp\min_zv#, e\tmp\max_zv#)					
					Else
	...
	If e\tmp\animtex Then
		...
		Select e\tmp\loadedtex
			Case false: EntityTexture e\ent, e\tmp\tex, e\tmp\texframe#
			Case true: EntityTexture e\ent, PARTICLETEXTURES(e\tmp\tex), e\tmp\texframe#
		End Select
	EndIf
...
For p.Particle = Each Particle
	...
	Else
		If p\emitter\frozen = False Then
			p\age = p\age + 1*d\Delta
			If p\emitter\tmp\align_to_fall Then p\rot# = p\emitter\tmp\align_to_fall_offset - ATan2(p\xv#, p\yv#)*d\Delta Else p\rot# = p\rot# + p\rot_vel#*d\Delta
			p\yv# = p\yv# - p\emitter\tmp\gravity#*d\Delta
			p\x# = p\x# + p\xv#*d\Delta
			p\y# = p\y# + p\yv#*d\Delta
			p\z# = p\z# + p\zv#*d\Delta
			...
			p\sx# = (p\sx# - p\emitter\tmp\size_sub#) * p\emitter\tmp\size_mult#
			p\sy# = (p\sy# - p\emitter\tmp\size_sub#) * p\emitter\tmp\size_mult#			
		EndIf
		...
		If p\emitter\tmp\realrgbenabled=False Then
		...
		Else
		VertexColor(p\emitter\surf,v1,p\emitter\tmp\r1, p\emitter\tmp\g1, p\emitter\tmp\b1,a#)
		VertexColor(p\emitter\surf,v2,p\emitter\tmp\r1, p\emitter\tmp\g1, p\emitter\tmp\b1,a#)
		VertexColor(p\emitter\surf,v3,p\emitter\tmp\r1, p\emitter\tmp\g1, p\emitter\tmp\b1,a#)
		VertexColor(p\emitter\surf,v4,p\emitter\tmp\r1, p\emitter\tmp\g1, p\emitter\tmp\b1,a#)
		EndIf
